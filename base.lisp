                     ...
                     *base-url*
                     ,@(loop 
                          ...
                          (cond
                            ...
                            (t
                             bit)))))
                  (optional-args (set-difference parameters required-args :test #'equal)))
             (values url-building-form required-args optional-args))))
  (let ((json
    ...
    (json:json-bind (results) json
      (with-open-file  (output "methods.lisp" :direction :output :if-exists :rename-and-delete)
        (let ((*print-case* :downcase))
        (format output "~&;; -*- mode: lisp; syntax: common-lisp; -*-")
        (format output "~&;;; Generated by build-methods in base.lisp (so don't edit)")
        (format output "~&(in-package \"ETSY\")")
       (loop for method in results
            do (json:json-bind (name description uri params type) method
                  (let ((parameters (loop for (key . type1) in params
                                       as type = (cond
                                                   ((string= type1 "enum(low, medium, high)")
                                                     "detail-level")
                                                   ((string= type1 "enum(up, down)")
                                                     "sort-order")
                                                   ((string= type1 "enum(created, ending)")
                                                     "sort-on-a")
                                                   ((string= type1 "enum(created, price)")
                                                     "sort-on-b")
                                                   ((string= type1 "enum(true, false)")
                                                     "boolean")
                                                   ((string= type1 "array(string)")
                                                    "array-of-strings")
                                                   ((string= type1 "array(int)")
                                                     "array-of-ints")
                                                   (t type1))
                                       collect (list (underscore-to-dash (symbol-name key))
                                                     (underscore-to-dash type)))))
                  (multiple-value-bind (url-building-form required-parameters optional-parameters) 
                  ...
                  (pprint
                   `(defun ,(camel-to-lisp name) (,@(mapcar #'car required-parameters)
                      ...
                      (let ((cgi-args (list (cons "api_key" *api-key*))))
                        ,@(loop for (name type) in optional-parameters
                             collect `(when ,name
                                        ...
                                        (push (cons ,(lisp-to-cgi name) (marshall-type ,type ,name))
                                              cgi-args)))
                        (demarshall-results (api-call ,url-building-form cgi-args) 
                                            ,type
