...
(defun marshall-array-of-strings (x) (format nil "~{~A~^,~}" (coerce 'list x)))
(defun marshall-array-of-ints (x) (format nil "~{~D~^,~}" (coerce 'list x)))
(defun marshall-color-triplet (x) (if (integerp x)
                                      (format nil "~X" x)
                                      (format nil "~D;~D;~D" (svref x 0)(svref x 1) (svref x 2))))
